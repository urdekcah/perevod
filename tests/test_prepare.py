...
class PipelineTestCase(unittest.TestCase):
    ...
    def write(self, lines, name="batch-01.txt", encoding="utf-8"):
        path = self.inbox / name
        path.write_text("\n".join(lines) + "\n", encoding=encoding)
        return path

    def write_pairs(self, pairs, name="batch-01.txt"):
        return self.write([f"{ru} ||| {ko}" for ru, ko in pairs], name=name)

    def run_pipeline(self, out="out", **overrides):
        options = {"inputs": (self.inbox,), "output_dir": self.root / out}
...
class ParsingTests(PipelineTestCase):
    def test_every_malformed_shape_is_named_by_file_and_editor_line_number(self):
        self.write(
            [
                "# batch header",
                "",
    ...
    def test_comments_blank_lines_a_bom_and_crlf_all_survive(self):
    ...
    def test_a_file_that_is_not_utf8_is_named_rather_than_crashing(self):
    ...
    def test_an_input_set_with_no_pairs_is_an_error(self):
    ...
    def test_a_non_txt_file_in_a_scanned_directory_is_reported_not_ignored(self):
...
class NormalizationTests(PipelineTestCase):
    def test_decomposed_and_precomposed_hangul_are_the_same_pair(self):
        target = "안녕하세요"
    ...
    def test_the_payload_keeps_the_author_punctuation_and_spacing_verbatim(self):
        source = "«Цитата»  —  тире"
    ...
    def test_russian_case_is_meaningful_so_it_is_never_folded(self):
...
class DeduplicationTests(PipelineTestCase):
    def test_a_repeated_pair_is_dropped_and_both_locations_are_reported(self):
    ...
    def test_one_source_with_two_translations_stops_the_run_by_default(self):
    ...
    def test_the_opt_in_keeps_conflicting_rows_together_in_one_split(self):
    ...
    def test_no_source_ever_straddles_a_split_boundary(self):
    ...
    def test_the_same_target_from_two_sources_is_ordinary_data(self):
...
class AlignmentGuardTests(PipelineTestCase):
    def test_a_wildly_uneven_pair_is_rejected_with_both_counts_and_the_option(self):
    ...
    def test_the_bound_can_be_raised_or_disabled(self):
        self.write([f"{'Слово ' * 40}||| 짧다"])

        for bound in (0.0, 200.0):
            with self.subTest(bound=bound):
                result = self.run_pipeline(
                    out=f"out-{bound}", max_length_ratio=bound, valid_fraction=0.0, test_fraction=0.0
                )
                self.assertEqual(result.manifest["counts"]["pairs_read"], 1)

    def test_the_optional_character_cap_is_off_until_it_is_set(self):
...
class DeterminismTests(PipelineTestCase):
    ...
    def test_reordering_the_input_leaves_the_splits_untouched(self):
    ...
    def test_two_input_files_give_the_same_splits_in_either_order(self):
    ...
    def test_changing_the_seed_moves_at_least_one_row(self):
        self.write_pairs(sample_pairs(30))
...
class OutputTests(PipelineTestCase):
    def test_a_split_asked_for_but_unrealizable_is_an_error_not_a_missing_file(self):
    ...
    def test_a_zero_fraction_produces_no_file_for_that_split(self):
    ...
    def test_the_manifest_records_the_build_without_volatile_content(self):
        self.write_pairs(sample_pairs(20))
        ...
        self.assertEqual(sorted(entry["name"] for entry in manifest["outputs"]),
    ...
    def test_fractions_are_range_checked(self):
        self.write_pairs(sample_pairs(20))

        for valid, test in ((0.6, 0.5), (-0.1, 0.1), (1.0, 0.0)):
            with self.subTest(valid=valid, test=test):
...
class RerunPolicyTests(PipelineTestCase):
    ...
    def test_a_non_empty_directory_stops_the_run_before_anything_is_touched(self):
    ...
    def test_overwrite_replaces_the_owned_files(self):
        result = self.run_pipeline(overwrite=True, seed=5)

        self.assertNotEqual(
            result.manifest["outputs"], self.first.manifest["outputs"]
        )

    def test_overwrite_removes_an_owned_split_the_new_build_does_not_produce(self):
        self.run_pipeline(overwrite=True, test_fraction=0.0)

        self.assertFalse((Path(self.first.output_dir) / TEST_FILENAME).exists())

    def test_overwrite_never_touches_a_file_this_command_does_not_own(self):
...
class ContainmentTests(PipelineTestCase):
    def test_a_path_with_parent_segments_writes_only_inside_the_resolved_target(self):
        self.write_pairs(sample_pairs(20))
    ...
    def test_a_symlinked_output_directory_resolves_to_its_target(self):
        self.write_pairs(sample_pairs(20))
        real = self.root / "real"
        real.mkdir()
        link = self.root / "link"
    ...
    def test_writing_into_the_hand_authored_corpus_is_refused(self):
        self.write_pairs(sample_pairs(20))
